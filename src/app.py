...
class EditDialog(ManualDialog):
    ...
    def init_extra_ui(self):
        ...
        # Insert the extra button above the Cancel button
        self.layout().insertWidget(self.layout().count() - 1, self.delete_button)
...
class MainWindow(QWidget):
    ...
    def on_item_selection_changed(self):
        ...
        if len(selected_items) > 0:
            ...
            selected_item = self.list_widget.itemWidget(selected_item)
            dialog = EditDialog()
            dialog.part_num_edit.setText(selected_item.part_number)
            dialog.qty_edit.setText(selected_item.qty)
            dialog.exec_()
            if dialog.result == 'del':
            ...
            if pn == '' or pq == '':
                ...
                self.list_widget.blockSignals(False)
                return
            selected_item.part_number = pn
            selected_item.qty = pq
            selected_item.part_number_label.setText(pn)
            selected_item.qty_label.setText(pq)
            self.list_widget.clearSelection()
        else:
            pass
